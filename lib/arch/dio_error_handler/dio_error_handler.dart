...
class DioErrorHandlerImpl<DE> implements DioErrorHandler<DE> {
  ...
  Future<CommonResponseError<DE>> _processDioError(DioError e) async {
    ...
    if (e.type == DioErrorType.connectionError) return const CommonResponseError.serverNotAvailable();
    //
    // if (e.type == DioErrorType.badResponse) {
    //   if (responseData['detail']) {
    //     switch
    //   }
    //   return CommonResponseError.customError(DefaultApiError(code: statusCode.toString(), name: responseData['detail']) as DE);
    // }
    ...
    if (statusCode == _HttpStatus.internalServerError) {
      return const CommonResponseError.serverError();
    }
