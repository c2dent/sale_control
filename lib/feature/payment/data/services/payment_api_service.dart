...
abstract class PaymentApiService {
  ...
  Future<Either<CommonResponseError<DefaultApiError>, List<PaymentSync>>> listSync(Map<String, String> params);
...
class PaymentApiServiceImpl extends PaymentApiService {
  ...
  @override
  Future<Either<CommonResponseError<DefaultApiError>, Payment>> update(String id, Map<String, dynamic> data) async {
  ...
  @override
  Future<Either<CommonResponseError<DefaultApiError>, List<PaymentSync>>> listSync(Map<String, String> params) async {
    List<PaymentSync> payments = [];

    final result = await _dioErrorHandler.processRequest(() => _client.get("/sync/payment/", queryParameters: params));
    if (result.isLeft) return Either.left(result.left);

    for (var item in result.right.data['results']) {
      payments.add(PaymentSync.fromJson(item));
    }
    return Either.right(payments);
  }
  ...
  Future<Either<CommonResponseError<DefaultApiError>, bool>> setSyncData(List<Map<String, dynamic>> data) async {
    final result = await _dioErrorHandler.processRequest(() => _client.post("/sync/payment/", data: data));
    if (result.isLeft) return Either.left(result.left);
